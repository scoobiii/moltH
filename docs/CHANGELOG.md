> **GOS3** · agente: `Gemini / ProtocolEngine` · papel: `GOS3 Multimodal Engine` (ver docs/team.md)
> fase: `fase 5 — MCP Protocol & Dataviz Stabilization Release` · data: `2026-08-28` · hora: `10:15:00 UTC`
> antes: CHANGELOG listava até a versão 1.3.0 (ADR-003 Runtime ID)
> depois: CHANGELOG atualizado com versão 1.4.0 incluindo suporte oficial ao Model Context Protocol (MCP v1.0), normalização de renderização Recharts e alinhamento de governança
> base: commit `gos3-core-v1.4`
> assinatura: `Gemini · GOS3 Multimodal Engine · GOS3 (v1.4.0)`
...
### 🚀 Adicionado
- **Model Context Protocol (MCP v1.0) Server & Client (`src/server/mcpService.ts`)**:
  - Servidor e cliente nativos MCP com transporte HTTP/SSE e JSON-RPC 2.0.
  - Conectores para **GitHub REST API v3** (leitura de repositórios, listagem e criação auditada de issues/PRs) e **Google Cloud Platform**.
  - Política Anti-Fabricação GOS3: Emissão de `claim: "auth_required"` com hash SHA-256 formal quando credenciais não estiverem configuradas, sem falsos positivos ou simulações opacas.
  - Nova interface de usuário em `src/components/connectors/ConnectorsModal.tsx` com catálogo interativo, testador de tokens PAT/GCP e executor auditado de tools MCP.
- **Endpoints de Conectores & MCP no Backend (`server.ts`)**:
  - Rotas `/api/mcp/servers`, `/api/mcp/tools`, `/api/mcp/execute` e streaming SSE em `/api/mcp/sse`.

### 🛡️ Corrigido / Hardening
- **Estabilização de Dimensionamento Recharts (`AgentActivityHeatmap.tsx` & `InteractiveChartEmbed.tsx`)**:
  - Resolução dos warnings repetitivos de renderização (`The width(0) and height(0) of chart should be greater than 0`).
  - Aplicação de `debounce={50}`, proporções `width="99%"`, alturas fixas e classes `min-w-[...]` para prevenir colapso de dimensões no ciclo de vida inicial do React e em transições de abas/modais.
- **Tolerância a Picos de Demanda na API Gemini (`src/server/agentRunner.ts`)**:
  - Fallback automático e transparente em situações de indisponibilidade momentânea (`503 UNAVAILABLE / High Demand`), preservando a responsividade contínua dos agentes.
