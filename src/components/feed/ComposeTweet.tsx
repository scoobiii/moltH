...
export const ComposeTweet: React.FC<Props> = ({
  ...
  // Suporte a colar arquivos/imagens da área de transferência (Ctrl+V)
        ...
        if (file) {
          ...
          reader.onload = (event) => {
            ...
            const newAtt: PostAttachment = {
              id: `att-img-${Date.now()}-${Math.random().toString(36).substr(2, 4)}`,
              type: "image",
              url: dataUrl,
              title: file.name || "Imagem Colada (Clipboard)",
              description: `Imagem PNG/JPG colada (${sizeKb} KB)`,
            };
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    Array.from(files).forEach((file: File) => {
      ...
      reader.onload = (event) => {
        const dataUrl = event.target?.result as string;
        const newAtt: PostAttachment = {
          id: `att-drop-${Date.now()}-${Math.random().toString(36).substr(2, 4)}`,
          type: isImg ? "image" : "document",
          url: dataUrl,
          title: file.name,
          description: `${isImg ? "Imagem" : "Documento"} (${sizeStr}) - ${file.type || "Arquivo"}`,
          sizeBytes: file.size,
          mimeType: file.type,
        };
        handleAddAttachment(newAtt);
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className={`p-4 border-b border-neutral-800 bg-neutral-950/90 relative transition-all duration-200 ${
        isDragging ? "ring-2 ring-purple-500 bg-purple-950/20 border-purple-500/50" : ""
      }`}
    >
      {isDragging && (
        <div className="absolute inset-0 bg-neutral-950/90 backdrop-blur-xs z-30 flex flex-col items-center justify-center border-2 border-dashed border-purple-500 rounded-xl m-2 pointer-events-none animate-in fade-in">
          <ImageIcon className="w-8 h-8 text-purple-400 animate-bounce mb-2" />
          <span className="text-sm font-semibold text-purple-200">Solte os arquivos ou imagens aqui para anexar</span>
          <span className="text-xs text-neutral-400 font-mono mt-1">Imagens, Documentos, Scripts</span>
        </div>
      )}
