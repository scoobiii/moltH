...
export const TweetCard: React.FC<Props> = ({
  ...
  const handleExecuteCodeLive = async () => {
    ...
    try {
      const lang = (post.codeArtifact.language || "").toLowerCase().trim();
      let toolName = "executeJavaScript";
      let params: Record<string, any> = { code: post.codeArtifact.code };

      if (lang === "bash" || lang === "sh" || lang === "shell" || lang === "zsh") {
        toolName = "executeBash";
        params = { command: post.codeArtifact.code, code: post.codeArtifact.code };
        ...
        // Smart syntax auto-detection for shell or python scripts
        const codeTrimmed = post.codeArtifact.code.trim();
        const firstLine = codeTrimmed.split("\n")[0] || "";
        if (
          firstLine.startsWith("#!") ||
          codeTrimmed.includes("git ") ||
          codeTrimmed.includes("ls -") ||
          codeTrimmed.includes("mkdir ") ||
          codeTrimmed.includes("echo ") ||
          codeTrimmed.includes("cat <<") ||
          codeTrimmed.includes("python3 ") ||
          codeTrimmed.includes("curl ") ||
          codeTrimmed.includes("grep ")
        ) {
          toolName = "executeBash";
          params = { command: post.codeArtifact.code, code: post.codeArtifact.code };
        } else if (
          codeTrimmed.includes("def ") ||
          codeTrimmed.includes("import sys") ||
          codeTrimmed.includes("import os") ||
      ...
      if (res.ok) {
        ...
        if (d.data?.stdout !== undefined && d.data?.stdout !== null && d.data?.stdout !== "") {
          formattedOutput = d.data.stdout;
          if (d.data.stderr) {
            formattedOutput += `\n[stderr]\n${d.data.stderr}`;
          }
        } else if (d.data?.stderr) {
          formattedOutput = `[stderr]\n${d.data.stderr}`;
        } else if (d.logs && d.logs.length > 0) {
          formattedOutput = d.logs.join("\n");
        } else if (d.data !== undefined && d.data !== null) {
          formattedOutput = typeof d.data === "object" ? JSON.stringify(d.data, null, 2) : String(d.data);
        } else {
          formattedOutput = "Executado com sucesso (sem saída no stdout).";
        }

        setLiveOutput(formattedOutput);
        setLiveDurationMs(d.executionTimeMs || d.data?.latencyMs || 0);
      } else {
        const errJson = await res.json().catch(() => ({}));
        setLiveOutput(`Erro na Sandbox: ${errJson.error || res.statusText}`);
      }
    } catch (e: any) {
