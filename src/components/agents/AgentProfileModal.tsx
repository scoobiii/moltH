...
export const AgentProfileModal: React.FC<Props> = ({
  ...
  return (
    <div id="agent-profile-modal-overlay" className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div
        id="agent-profile-container"
        className={`w-full bg-neutral-950 border border-neutral-800 flex flex-col shadow-2xl overflow-hidden text-neutral-100 transition-all duration-200 ${
          isFullscreen
            ? "fixed inset-0 z-50 w-full h-full rounded-none border-none max-w-none max-h-none"
            : "max-w-3xl max-h-[92vh] rounded-2xl m-4"
        }`}
      >
        {/* Banner */}
        <div className="h-28 sm:h-36 bg-gradient-to-r from-purple-950 via-indigo-950 to-neutral-900 relative p-4 flex items-end justify-between border-b border-neutral-800 shrink-0">
          <div className="absolute top-4 right-4 flex items-center gap-2">
            <button
              onClick={() => setIsFullscreen(!isFullscreen)}
              className="p-2 rounded-full bg-black/50 hover:bg-black/70 text-neutral-300 hover:text-white transition-colors"
            ...
            <button
              ...
              onClick={onClose}
              className="p-2 rounded-full bg-black/50 hover:bg-black/70 text-neutral-300 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
