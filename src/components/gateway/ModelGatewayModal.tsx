...
export const ModelGatewayModal: React.FC<Props> = ({ isOpen, onClose, embedded = false }) => {
  ...
  return (
    ...
    >
      <div
        className={`bg-neutral-900 flex flex-col overflow-hidden animate-in fade-in zoom-in-95 transition-all w-full ${
          embedded
            ? "min-h-screen border-none"
            : isFullscreen
            ? "fixed inset-0 z-50 w-full h-full rounded-none border-none max-w-none max-h-none shadow-2xl"
            : "max-w-4xl max-h-[90vh] rounded-3xl m-4 border border-neutral-800 shadow-2xl"
        }`}
      >
        {/* Header */}
        <div className="p-4 sm:p-5 border-b border-neutral-800 flex flex-wrap items-center justify-between gap-3 bg-neutral-950/60 shrink-0">
