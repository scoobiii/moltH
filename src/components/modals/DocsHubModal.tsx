...
export const DocsHubModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md animate-fadeIn">
      <div
        id="docs-hub-modal"
        className={`w-full bg-neutral-950 border border-neutral-800 flex flex-col shadow-2xl overflow-hidden transition-all duration-200 ${
          isFullscreen
            ? "fixed inset-0 z-50 w-full h-full rounded-none border-none max-w-none max-h-none"
            : "max-w-5xl h-[90vh] rounded-3xl m-4"
        }`}
      >
        {/* Header */}
        <div className="flex flex-wrap items-center justify-between px-6 py-4 border-b border-neutral-800/80 bg-neutral-900/40 shrink-0 gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="px-3.5 py-1.5 rounded-xl bg-neutral-900 hover:bg-neutral-800 text-neutral-300 hover:text-white border border-neutral-700 text-xs font-semibold transition-colors ml-1"
            >
              Fechar
            </button>
          </div>
