...
export const AuthModal: React.FC<Props> = ({
  ...
  const availableAgents = [
    ...
    { handle: "@ClaudeOpus", name: "Claude Opus Formal Engine" },
    { handle: "@GPT4o", name: "GPT-4o Multimodal Lead" },
    ...
    try {
      const res = await fetch("/api/auth/login", {
        ...
        body: JSON.stringify({
          ...
          bio: `Conta Google autenticada (sobrinhoSJ@gmail.com). Operador GOS3.`,
          authProvider: "google",
        }),
      });
      ...
      setSuccessMsg(`Autenticado com sucesso via Google SSO (@${data.user.handle})!`);
      onLoginSuccess(data.user);
      setTimeout(() => {
        setSuccessMsg("");
        onClose();
  ...
  return (
    <div id="auth-modal-backdrop" className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md animate-in fade-in">
      <div
        id="auth-login-modal"
        className={`w-full bg-neutral-900 border border-neutral-800 shadow-2xl overflow-hidden text-neutral-100 flex flex-col transition-all duration-200 ${
          isFullscreen
            ? "fixed inset-0 z-50 w-full h-full rounded-none border-none max-w-none max-h-none"
            : "max-w-2xl max-h-[92vh] rounded-3xl m-4"
        }`}
      >
        {/* Header */}
        <div className="px-6 py-4 border-b border-neutral-800 flex flex-wrap items-center justify-between gap-3 bg-neutral-950/80 shrink-0">
            ...
            <div className="space-y-4 text-xs">
              <div className="p-4 rounded-2xl bg-neutral-950 border border-neutral-800 flex items-center gap-3.5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-neutral-100 text-sm">Google Identity Services (OAuth 2.0)</div>
                  <div className="text-[11px] text-neutral-400">Autenticação oficial via Google Cloud / Firebase Auth</div>
                </div>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-neutral-950 border border-neutral-800 space-y-2">
                <div className="flex items-center gap-2 text-emerald-400 font-semibold text-xs">
                  ...
                  <span>Garantia de Segurança Anti-Phishing (Zero-Trust GOS3)</span>
                </div>
                <ul className="space-y-1.5 text-[11px] text-neutral-300 list-disc list-inside">
                  <li>O login acontece diretamente na infraestrutura segura do Google (<code className="font-mono text-purple-300">accounts.google.com</code>).</li>
                  <li><strong>Zero captura de credenciais</strong>: Este app nunca solicita que você digite e-mails ou senhas em campos de texto de terceiros.</li>
                  <li>Sua sessão e identidade são transferidas via tokens JWT criptografados com verificação de assinatura.</li>
                </ul>
              </div>
              ...
              <button
                ...
                disabled={loading}
                className="w-full py-3 rounded-2xl bg-white hover:bg-neutral-100 text-neutral-900 font-bold text-xs flex items-center justify-center gap-2 shadow-lg transition-all disabled:opacity-50 cursor-pointer"
              >
                ...
                <span>{loading ? "Conectando ao Google..." : "Continuar com o Google (Single Sign-On)"}</span>
