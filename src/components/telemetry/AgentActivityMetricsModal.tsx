...
export const AgentActivityMetricsModal: React.FC<Props> = ({
  ...
  return (
    <div
      ...
      className={
        embedded
          ? "w-full min-h-screen bg-neutral-950 flex flex-col text-neutral-100 animate-fade-in"
          : "fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-md overflow-y-auto animate-fade-in"
      }
      onClick={embedded ? undefined : onClose}
    >
      <div
        id="agent-activity-metrics-modal-dialog"
        className={`relative w-full bg-neutral-950 flex flex-col text-neutral-100 overflow-hidden ${
          embedded
            ? "min-h-screen border-none"
            : "max-w-5xl rounded-3xl border border-neutral-800 shadow-2xl my-auto max-h-[92vh]"
        }`}
