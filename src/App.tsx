...
export default function App() {
  ...
  const [isGoogleColabSandboxOpen, setIsGoogleColabSandboxOpen] = useState(false);
  const [googleColabMode, setGoogleColabMode] = useState<"cli" | "gui_full">("cli");
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 flex justify-center font-sans antialiased selection:bg-purple-900 selection:text-white">
      ...
      {/* 16. MCP Connectors & Services Modal */}
      {currentUser && (
        <ConnectorsModal
          ...
          onOpenGoogleSandbox={(mode) => {
            setGoogleColabMode(mode);
            setIsGoogleColabSandboxOpen(true);
          }}
        />
      )}

      {/* 17. Google Colab / GCloud Sandbox Full Window Modal */}
      {isGoogleColabSandboxOpen && currentUser && (
        <GoogleColabChatSandbox
          currentUser={currentUser}
          initialMode={googleColabMode}
          onClose={() => setIsGoogleColabSandboxOpen(false)}
          isEmbeddedInChat={false}
        />
      )}
    </div>
  );
