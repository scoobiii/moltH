...
${executableCode}
  ...
  /**
   * OpenClaw Bash / Alpine Linux Execution Engine
   */
  static async executeBash(
    command: string,
    optionsOrTimeout: number | { timeoutMs?: number; cwd?: string; customEnv?: Record<string, string>; envProfile?: string } = 4000
  ): Promise<SandboxToolResult> {
    const res = await OpenClawService.executeBash(command, optionsOrTimeout);
    return {
