...
import { AgentThoughtLog, InteractiveChartData, CodeExecutionArtifact, Post, ThoughtStep, UserAccount, ModelProviderId, ExternalSideEffectReceipt, PostAttachment } from "../types";
...
export class AgentRunner {
  ...
  static async runAgent(
    ...
    // Aggregate all attachments from current message and thread
    const allAttachments: PostAttachment[] = [...attachments];
    ...
    // 1.5. Multimodal Object Mask Detection, Task Extraction & Context Classification
        ...
        if (att.type === "image" && att.url.startsWith("data:image/")) {
          ...
          if (match) {
            ...
            detectedEntities.push({
              file: att.title || "image.png",
              classification: "UI Screen / Task / Error Modal",
              maskBounds: [0.12, 0.18, 0.88, 0.82],
              inferredTasks: ["Auditoria de tela", "Resolução de erro de commit/push", "Execução de backlog"],
            });
          }
        } else if (att.type === "archive" || att.url.endsWith(".zip") || att.url.endsWith(".tar") || att.url.endsWith(".tar.gz")) {
          detectedEntities.push({
            file: att.title || "archive.tar.gz",
            classification: "Archive Tar/Zip Codebase",
            filesContained: att.metadata?.archiveFilesCount || 12,
            inferredTasks: ["Inspeção de estrutura de arquivos", "Validação de dependências"],
          });
        }
      }

      steps.push({
        id: `step-${Date.now()}-vision-ocr`,
        title: `Detecção de Máscara de Objetos & Classificação Multimodal`,
        description: `Processados ${allAttachments.length} anexos (${geminiImageParts.length} partes de imagem direta, ${allAttachments.filter(a => a.type === 'archive').length} pacotes tar/zip). Extraídas tarefas e notas de backlog em tempo real.`,
        toolName: "objectMaskDetectAndContextClassifier",
        inputArgs: { attachmentsCount: allAttachments.length },
        outputResult: detectedEntities,
        status: "success",
        timestamp: new Date().toISOString(),
        latencyMs: Date.now() - visionStartTime,
        decisionFactor: "Multimodal Vision Input",
        confidenceScore: 0.98,
        rationale: "Extração direta de pixels e metadados contextuais com suporte completo a imagens e archives",
      });
    }

    // 2. Build contextual prompts with memories & attachments
    const systemPrompt = buildAgentSystemPrompt(agent, recalledMemories);
    const contextPrompt = buildThreadContextPrompt(triggerContent, threadHistory, mentionedHandles, attachments);
    ...
    if (provider === "gemini" && ai) {
      ...
      for (const candidate of uniqueCandidates) {
        ...
        try {
          const contentsPayload: any = geminiImageParts.length > 0
            ? [...geminiImageParts, contextPrompt]
            : contextPrompt;
