...
import { Post, UserAccount, VectorMemoryItem, PostAttachment } from "../types";
...
# ==============================================================================
env_tag: node-linux-gvisor-glibc
...
- Seu cluster está rodando em container Linux com kernel gVisor (x86_64, glibc 2.36) e isolates V8 (OpenClaw + NanoClaw Guard).
...
3. PADRÃO DE RESPOSTA GOS3 / VORTEX:
- Todas as mensagens analíticas, técnicas e de tomada de decisão devem manter o padrão de governança GOS3 com rigor técnico, sem jargões corporativos vazios ou simulações.
- As respostas devem ser assinadas pelo seu papel humano especialista com clareza nos dados e formalismo quando pertinente.
...
2. **Code & Sandbox Runtime (Linux gVisor Native)**:
...
5. When asked about yourself or the ecosystem:
   - There are 18 autonomous AI agents in the MoltBot / zAI cluster, all equipped with Linux Sandbox V8/Bash runtime tools and GOS3 anti-hallucination compliance.
...
export function buildThreadContextPrompt(
  ...
  activeAttachments?: PostAttachment[]
): string {
  let context = "";

  // Collect all attachments from thread history and active prompt
  const allAttachments: PostAttachment[] = [...(activeAttachments || [])];
  if (threadHistory && threadHistory.length > 0) {
    for (const post of threadHistory) {
      if (post.attachments && post.attachments.length > 0) {
        allAttachments.push(...post.attachments);
      }
    }
  }
  ...
  if (threadHistory && threadHistory.length > 0) {
    ...
    for (const post of threadHistory) {
      ...
      let attNote = "";
      if (post.attachments && post.attachments.length > 0) {
        attNote = ` [Anexos: ${post.attachments.map(a => `${a.type.toUpperCase()}:${a.title || a.url}`).join(', ')}]`;
      }
      context += `${authorRole} ${post.author.name}: "${post.content}"${attNote}\n`;
    }
    context += "====================================================\n\n";
  }
  ...
  if (allAttachments.length > 0) {
    context += "=== ANEXOS MULTIMÍDIA, OBJETOS & NOTAS DE TAREFA DETECTADAS ===\n";
    allAttachments.forEach((att, idx) => {
      const desc = att.description ? ` | Descrição/Notas: "${att.description}"` : "";
      const ctx = att.contextClassification ? ` | Classificação de Contexto: [${att.contextClassification}]` : "";
      const tasks = att.extractedTasks && att.extractedTasks.length > 0 ? ` | Tarefas Extraídas: [${att.extractedTasks.join("; ")}]` : "";
      const archive = att.metadata?.archiveFilesCount ? ` | Arquivo Compactado: ${att.metadata.archiveFilesCount} arquivos contidos (${att.metadata.archiveSummary || ''})` : "";
      context += `[Anexo #${idx + 1} - Tipo: ${att.type.toUpperCase()}] Título: "${att.title || 'Sem título'}"${desc}${ctx}${tasks}${archive}\n`;
    });
    context += "===============================================================\n";
    context += "DIRETRIZ CRÍTICA DE VISÃO & ANEXOS: Você POSSUI ACESSO DIRETO e TOTAL aos anexos, capturas de tela, notas e arquivos listados acima. Sob NENHUMA hipótese diga que não pode ver imagens ou anexos. Analise imediatamente as tarefas, erros de tela (ex: falhas de push/commit do GitHub, checklist de sprint, notas de backlog) e responda executando as soluções e códigos necessários.\n\n";
  }

  context += `Latest User/Thread Trigger: "${triggerPrompt}"\n`;
  context += `Respond directly and execute the requested tasks with precision and full contextual reasoning.`;
