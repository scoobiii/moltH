...
import { multiAgentPipeline } from "./src/server/orchestrator/multiAgentPipeline";
...
async function startServer() {
  ...
  app.post("/api/posts", async (req, res) => {
    try {
      ...
      if (author.isAgent) {
        ...
        const agentResult = await AgentRunner.runAgent(author, content, threadHistory, mentions, attachments || []);
          ...
          for (const handle of mentions) {
            ...
            if (agent && agent.isAgent && agent.id !== author.id) {
              try {
                ...
                const replyResult = await AgentRunner.runAgent(agent, created.content, thread, mentions, created.attachments || []);
  ...
  app.post("/api/sandbox/execute", async (req, res) => {
    ...
    try {
      if (toolName === "executeBash" || toolName === "bash" || toolName === "sh" || toolName === "shell") {
        result = await AgentSandbox.executeBash(params?.command || params?.code || "ls -la");
      } else if (toolName === "executePython" || toolName === "executePythonSim" || toolName === "python_sandbox" || toolName === "python" || toolName === "py") {
        result = await AgentSandbox.executePython(params?.code || params?.script || params?.command || "print('Hello Python 3')");
      } else if (toolName === "executeJavaScript" || toolName === "executeTypeScript" || toolName === "js_sandbox" || toolName === "ts_sandbox" || toolName === "typescript" || toolName === "javascript" || toolName === "js" || toolName === "ts") {
        result = AgentSandbox.executeJavaScript(params?.code || params?.script || params?.command || "console.log('Hello Sandbox');");
  ...
  // Dedicated Linux Terminal Endpoint for Agents & Sandbox Lab (Alpine & Linux Tool Interfacing)
  app.post("/api/sandbox/terminal", async (req, res) => {
    ...
    try {
      ...
      const cmdToRun = (command || script || "").trim();
      if (!cmdToRun && !toolInvocation) {
        return res.status(400).json({ error: "Campo 'command' ou 'script' é obrigatório." });
      }

      let execResult: any;

      if (toolInvocation && toolInvocation.toolName) {
        // Direct tool invocation bridge through terminal
        const { toolName, args = {} } = toolInvocation;
        if (toolName === "runtimeCheck") {
          execResult = await AgentSandbox.runtimeCheck(args);
        } else if (toolName === "executePython") {
          execResult = await AgentSandbox.executePython(args.code || "print('Alpine Linux Python 3 probe')");
        } else if (toolName === "analyzeRepository") {
          execResult = await AgentSandbox.analyzeRepository(args);
        } else if (toolName === "inspectNanoClawRuntime") {
          execResult = AgentSandbox.inspectNanoClawRuntime(args);
        } else if (toolName === "calculateEnergyBESS") {
          execResult = AgentSandbox.calculateEnergyBESS(args);
        } else if (toolName === "analyzeMarketCrypto") {
          execResult = AgentSandbox.analyzeMarketCrypto(args);
        } else {
          execResult = await AgentSandbox.executeBash(cmdToRun || `echo "Tool: ${toolName}"`, {
            timeoutMs,
            cwd: workingDir,
            customEnv,
            envProfile,
        ...
        execResult = await AgentSandbox.executeBash(cmdToRun, {
          timeoutMs,
          cwd: workingDir,
          customEnv,
          envProfile,
      ...
      // Persistence log for agent telemetry
      persistence.saveNx1Execution({
        agent_id: agentHandle,
        prompt: `Terminal Exec [${envProfile.toUpperCase()}]: ${cmdToRun || toolInvocation?.toolName}`,
        status: execResult.success ? "success" : "failed",
        latency_ms: durationMs,
        evidence_hash: execResult.evidenceHash,
        output: execResult.data?.stdout || (typeof execResult.data === "object" ? JSON.stringify(execResult.data) : execResult.data) || "Executed",
        tool_calls: ["executeBash"],
        metrics: {
      ...
      const responsePayload = {
        ...
        workingDir,
        envProfile,
        stdout: execResult.data?.stdout !== undefined ? execResult.data.stdout : (typeof execResult.data === "object" ? JSON.stringify(execResult.data, null, 2) : String(execResult.data || "")),
        stderr: execResult.data?.stderr || "",
        exitCode: execResult.data?.exitCode ?? (execResult.success ? 0 : 1),
        ...
        system: {
          ...
          cwd: workingDir && workingDir !== "." ? workingDir : process.cwd(),
          envProfile,
          gos3_env_tag: envProfile === "alpine" ? "node-linux-proot-alpine" : "node-linux-container",
        },
      };
      ...
      res.status(500).json({
        ...
        error: err.message,
        stdout: "",
        stderr: `Erro na execução do terminal: ${err.message}`,
        exitCode: 1,
        logs: [`[Terminal Error] ${err.message}`],
  ...
  app.get("/api/orchestrator/tasks", (_req, res) => {
    try {
      const tasks = multiAgentPipeline.listTasks();
  ...
  app.post("/api/orchestrator/tasks", (req, res) => {
    try {
      const { title, description, initiator, steps, initialMemory } = req.body;
      if (!title || !steps || !Array.isArray(steps)) {
        return res.status(400).json({ success: false, error: "title and steps array are required" });
      }
      const task = multiAgentPipeline.createTask({
        title,
        description: description || "",
        initiator: initiator || { id: "user-sobrinho", handle: "sobrinhoSJ", type: "human" },
        steps,
        initialMemory,
      });
  ...
  app.post("/api/orchestrator/tasks/:id/step", async (req, res) => {
    try {
      const task = await multiAgentPipeline.executeNextStep(req.params.id);
  ...
  app.post("/api/orchestrator/test-acceptance", async (_req, res) => {
    try {
      const testResult = await runMultiAgentAcceptanceTest();
  ...
  app.post("/api/mcp/yai/chaos-matrix", async (_req, res) => {
    try {
      const summary = await yaiMcpChaosRunner.runFullMatrix();
